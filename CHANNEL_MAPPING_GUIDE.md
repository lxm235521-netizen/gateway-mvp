...
其中 `optimizer_concurrency`（默认 8，上限 500）和 `optimizer_queue_wait_ms`（默认 300000）是全局专属项，只在这一页配置。
...
1. **排队限流**：网关自身限制同时发往优化服务的请求数，默认 **8**（上限 500），可在后台「提示词优化」页调整。
   超出的请求排队等待，不直接撞服务端上限。调到上限相当于基本不排队。
...
- 并发数不是越小越安全：并发压得过低会让请求大量排在队列里，反而更容易撞上排队上限而放弃优化。
  建议按实际吞吐设置，只有在日志里反复出现 `Concurrency limit exceeded` 时才调低；上限 500 基本等价于不排队。
  账号并发额度充足时可以直接调高，不必保守。
