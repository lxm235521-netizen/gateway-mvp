        ...
        try {
            ...
            for (const binding of bindings || []) {
                ...
                await db.run(`INSERT INTO model_bindings
                    (logical_model_id, channel_id, route_path, poll_path, api_key, is_async, proxy_content, error_passthrough, req_mapping, resp_mapping, poll_mapping, weight, status)
        ...
        try {
            ...
            if (Array.isArray(bindings)) {
                for (const binding of bindings) {
                    const item = normalizeBinding(binding);
                    if (binding.id) {
                        await db.run(`UPDATE model_bindings SET channel_id=?, route_path=?, poll_path=?, api_key=?, is_async=?, proxy_content=?, error_passthrough=?,
